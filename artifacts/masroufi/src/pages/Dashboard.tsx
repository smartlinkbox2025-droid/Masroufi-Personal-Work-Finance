...
export default function Dashboard() {
  ...
  const primaryCards = [
    ...
    { label: 'صافي التدفق النقدي', amount: summary?.net ?? 0, Icon: Wallet, tone: (summary?.net ?? 0) >= 0 ? 'text-primary' : 'text-destructive', bg: 'bg-primary/10' },
  ];
  return (
...
function MiniCard({ label, value, negative }: { label: string; value: number; negative?: boolean }) { return <div className="rounded-2xl border border-border bg-card p-4 shadow-sm"><p className="text-xs text-muted-foreground">{label}</p><p className={`mt-2 text-lg font-black ${negative ? 'text-destructive' : 'text-primary'}`} dir="ltr">{formatSAR(value)}</p></div>; }
function SectionTitle({ title, href }: { title: string; href: string }) { return <div className="mb-4 flex items-center justify-between gap-3"><h2 className="font-bold">{title}</h2><Link href={href} className="text-xs font-bold text-primary hover:underline">عرض الكل</Link></div>; }
